...
export enum CameraControlFlags {
  NONE = 0,
  MOVEMENT = 1,
  MOUSE = 2,
  SPRINT_AND_DUCK = 4,
...
class Camera extends EventEmitter {
  ...
  async _onMessage(message: AppMessage) {
  ...
  async _onCameraRays(cameraRays: AppCameraRays) {
    // do nothing if not subscribed
    if (!this.isSubscribed) {
      return;
    }
    ...
    if (this.cameraRays.length > 10 && this.cameraSubscribeInfo) {
  ...
  async _renderCameraFrame(
    ...
    for (
      let B = new IndexGenerator(1337), R = width * height - 1;
      R >= 1;
      R--
    ) {
      let C = 2 * R,
   ...
   */
  async move(buttons: number, x: number, y: number) {
  ...
  isAutoTurret(): boolean {
    ...
    return (
      this.cameraSubscribeInfo !== null &&
      (this.cameraSubscribeInfo.controlFlags & crosshairControlFlag) ===
        crosshairControlFlag
    );
  }
}
